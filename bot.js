...
bot.on('message', function (user, userID, channelID, message, evt) {
    var words = new Array('weqwewqe', 'AWDwADAWDsda', 'ASdadds', 'assaqdas', 'saddasxas');
    for (var i = 0; i < words.length-1; i++) {
      if (message.toLowerCase().includes(words[i])){
        logger.info("word was said by: '" + user + "' in channel: '" + channelID + "'")
        bot.sendMessage({
            to: channelID,
            message: "<" + ':dasdasdad:693764159925518357' + ">"
        });
      }
    }
    if (message.includes('are now married') && userID == 627116322097987624){
      var word = message.split(' ');
      if (word[4].includes("are")){
          logger.info(userID + " got married to " + word[3])
      }else{
          logger.info(userID + " got married to " + word[3] + " " + word[4])
      }
      bot.sendMessage({
          to: channelID,
          message: "<" + ':PogU:693440185379192833' + ">"
      });
    }
    if (message.substring(0, 1) == '!') {
        ...
        switch(cmd) {
            ...
            case 'rollcall':
                bot.sendMessage({
                    to: channelID,
                    message: 'roll calls <@' + userID + '>'
                });
                logger.info('roll call by' + userID + '>');
            break;
            case 'pogu':
            bot.sendMessage({
                to: channelID,
            ...
            case 'fact':
              ...
              bot.sendMessage({
                to: channelID,
                message: 'Here is a random fact for you <@' + userID + ">" + ',' + "\n" + '\u0022' + fact + '\u0022'
              });
            break;
            case 'paul':
            bot.sendMessage({
              to: channelID,
              message: "https://hydra.mediasearch.verizon.com/ResourcesFiles/ImageLibrary/FiosTV/UnBranded/H/169_Lrg/TVNSONYHD.COM_TVNH0035913702144361.jpg?s=1440x810",
              file: ""
              });
            break;
            case 'insult':
...
function getFact(){
  var useless_facts = new Array("Most American car horns honk in the key of F.",
    ...
    var int = getRandomInt(useless_facts.length - 1);
  return useless_facts[int];
}
function getInsult(){
  var insult_list = new Array('Mirrors cant talk, lucky for you they cant laugh either.', 'I dont believe in plastic surgery, But in your case, Go ahead.', 'People like you are the reason we have middle fingers.', 'Tell me… Is being stupid a profession or are you just gifted?', 'Why Dont You Slip Into Something More Comfortable. Like A Coma?', 'When your mom dropped you off at the school, she got a ticket for littering.', 'Me pretending to listen should be enough for you.', 'Zombies eat brains. Youre safe.', 'Whats the point of putting on makeup, a monkey is gonna stay a monkey.', 'Id agree with you but then wed both be wrong.', 'Your intelligence is my common sense.', 'No need for insults, your face says it all.', 'If I say something that offends you, let me know so I can do it again later.', 'Your lips are moving, but all I hear is “blah blah blah…”', 'If youre talking behind my back then youre in a perfect position to kiss my a**!', 'Youre so ugly that when you cry,  the tears roll down the back of your head…just to avoid your face.', 'Wow! You have a huge pimple in between your shoulders! Oh wait thats your face.', 'My mom says pigs dont eat biscuits… So I better take that one out of your hand.', 'Ill try being nicer, if you try being smarter.', 'Act your age not your shoe size.', 'Its not that you are weird…its just that everyone else is normal.', 'Scientists are trying to figure out how long human can live without a brain. You can tell them your age.', 'Its not that Im smarter than you, its just that youre dumber than everyone else.', 'Jealousy is a disease…get well soon!', 'Stupidity is not a crime so you are free to go.', 'Yes, you do have a right to your opinion…And I have a right to mine. And my opinion is that your opinion is ridiculously stupid!!', 'Whatever permission you thought you had to speak to me, I hereby remove.', 'Everyone has the right to be stupid, but youre abusing the privilege.', ' ', 'Just keep talking, I yawn when Im interested.', 'You know most days when I look in a mirror I feel ugly, but when I look at you I feel lucky.', 'Your age doesnt lie. Neither does that face.')
  var int = getRandomInt(insult_list.length - 1);
  return insult_list[int];
}
